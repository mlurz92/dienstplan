...
test('month transition stability is loaded after the spectrum director', () => {
  const spectrumImport = uiControls.indexOf("import './color-director.js?v=20260803.5';");
  const stabilityImport = uiControls.indexOf("import './month-transition-stability.js?v=20260803.5';");
...
test('every synchronization ends the base animation and hands the colour to the director', () => {
  assert.match(stability, /import \{ applyMonthTheme \} from '.\/theme\.js\?v=20260803\.5';/);
...
test('printing settles the director transition instead of the base theme', () => {
  ...
  assert.match(app, /import \{ applySpectrumProfile \} from '\.\/color-director\.js\?v=20260803\.5';/);
});
