 ...
 *
 * Die browserseitigen Konstruktionsfunktionen bleiben als deterministischer
 * Warmstart, Offlinefallback und unabhängige Heuristikreferenz verfügbar. Der
 * reguläre Studiolauf wird durch `auto-plan-runner.js` orchestriert und nutzt
 * den nativen CP-SAT-Dienst, sobald dessen Cloudflare-Binding bereitsteht.
 */
...
export const AUTO_PLAN_STAGES = Object.freeze([
  ...
  Object.freeze({ id: 'minimal-relaxation', title: 'Minimale Relaxierung', detail: 'Nur nach nachgewiesenem oder budgetbedingt ungelöstem Strict-Pfad werden freigegebene Ausnahmen lexikografisch minimiert.' }),
...
function annotate(result) {
  ...
  result.engineRevision = AUTO_PLAN_REVISION;
  result.metrics ||= {};
...
export async function constructAutoPlan(parameters) {
  return annotate(await V85.constructAutoPlan(parameters));
}

export async function perfectAutoPlan(parameters) {
  return annotate(await V85.perfectAutoPlan(parameters));
}

export async function buildAutoPlan(parameters) {
  return annotate(await V85.buildAutoPlan(parameters));
}
